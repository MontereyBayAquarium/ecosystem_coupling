...
model {
  ...
  for(t in 1:N){  
    ...
    pi_obs[t] ~ dirichlet(pi[t] * tau[t]) ; // Observed foraging effort allocation by year
  }
  // Priors:
  ...
  for(i in 1:(K-1)){
    ...
    L[2:N,i] ~ normal( L[1:(N-1),i], sig_L) ;
  }  
  L[1:N,K] ~ normal(0,.05) ; // "other" prey category assumed to remain constant  
}
//
generated quantities {
  ...
  for(t in 1:N){
    ...
    Ebar[t] = sum(pi[t] .* R ) ;
    U_prd[t] = pi[t][1] * (R[1] / G[1]) ;
    M_prd[t] = pi[t][2] * (R[2] / G[2]) ;
    if(t > N_base[2]){
      // Alternative scenario 1: urchins remain fixed at avg of base years
      lam[1][1] = 1 - exp(-exp( normal_rng(mean(L[iib,1]), sd(L[iib,1])) )) ;
      // Alternative scenario 2: mussels remain fixed at avg of base years
      lam[2][2] =  1 - exp(-exp( normal_rng(mean(L[iib,2]), sd(L[iib,2])) )) ;
    ...
    for(j in 1:3){
      ...
      Ebar_alt[j][t] = sum(pi_alt[j] .* R ) ;
      U_prd_alt[j][t] = pi_alt[j][1] * (R[1] / G[1]) ;
      M_prd_alt[j][t] = pi_alt[j][2] * (R[2] / G[2]) ;
    }
  }
}
